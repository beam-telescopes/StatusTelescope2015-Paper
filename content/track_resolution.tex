...
\noindent
The minimum of equation~(\ref{eq:globalchi2}) is then calculated to determine the intrinsic sensor resolution $\sigmai$.
The results for both the narrow plane spacing of $\dz = 20\,\milli\meter$ and the wide spacing of $\dz = 150\,\milli\meter$ are shown in figure~\ref{fig:smiley}.
In both cases, an error of $0.5\,\milli\meter$ on the plane distance $\textrm{d}z$ is assumed.
The measured residual widths are larger at every telescope plane for the wide configuration in comparison to the narrow one and also towards lower beam energies (not shown here). 
...
In the former case, the geometric scaling factor is larger in the wide configuration, in the ladder case $\sigma_{\textrm{MS}}$ increases with decreasing beam energy. 
...
The difference in efficiency for different energies and for different plane spacings is about 1\,\%. % and stems from multiple scattering and different extrapolation lengths due to differing $\dz$.
...
Measurements by Behr~\cite{ref:j.behrmeasurements} taken at $\noise\,>\,10$ show comparable results of $\sigma_{\textrm{M26}} = (4.35\,\pm\,0.10)\,\micro\meter$. %w/o Joerg
...
\begin{figure}[tbp]
  \centering
  \includegraphics[width=0.49\textwidth]{figures/CalcResoVsDzdut_Desy}\put(-165,35){(A)}
  \includegraphics[width=0.49\textwidth]{figures/CalcResoVsDzdut_Cern}\put(-165,35){(B)}
