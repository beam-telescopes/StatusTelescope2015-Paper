...
Alignment constants are determined to sub micron precision and are hence much smaller than the intrinsic resolution of the detectors used. 
...
In the formalism of GBL, a track refit is performed to an initial trajectory and the covariance matrix of each track parameter is determined. 
...
%(1)~Scatterers, the origin of kinks in the trajectory, are thin, i.e.\ no spatial offsets within the scatterer are taken into account. 
%(2)~A thick scatterer, like the air between the planes, is modelled by two thin scatterers. 
%(3)~The offsets of the local track parameters are small and propagate linearly. 
%(4)~Uncertainties are Gaussian distributed, i.e.\ residual and kink distributions.
\begin{itemize}
 \item Scatterers, the origin of kinks in the trajectory, are thin, i.e.\ no spatial offsets within the scatterer are taken into account. 
 \item A thick scatterer, like the air between the planes, is modelled by two thin scatterers. 
 \item The offsets of the local track parameters are small and propagate linearly. 
 \item Uncertainties are Gaussian distributed, i.e.\ residual and kink distributions.
\end{itemize}
...
Additionally, the contribution of air is accounted for as two scatterers. 
...
A width differing from one originates from an under- or overestimated residual prediction $\sqrt{\sigmai^2 - \sigmatb^2}$, which in turn stems from an inaccurate estimate of either the intrinsic resolution
 or the rack resolution. 
The latter can be a cause of an inaccurate inputs like the material budget or the assumed deflection angles variance. 
...
% METHODS YEILD SIMILAR RESULTS, also in uncerts
...
As the intrinsic resolution is not a priori known, an initial estimate of the intrinsic resolution $\sigmahat$ is used as an input to the GBL track fitting. 
...
The width of the pull distributions is iteratively used to update the estimate $\sigmahat$, which is used equally for all six planes. 
...
If the pulls distribution resembles $N(0,1)$, equations~(\ref{eq:telescoperesolutionequation1}) and~(\ref{eq:telescoperesolutionequation2}) hold and the systematic uncertainties of the intrinsic resolution
 are estimated based on uncertain beam energy, uncertain deflection angle variance and variation in fit range. 
...
The original systematic uncertainty of the scattering distribution width $\Theta_{0}$ of $11\,\%$ is experimentally constrained to 3.5\,\%, as is shown in section~\ref{sec:measurements}. 
...
It shows, that the choice of biased track fits results in similar systematic uncertainties compared to unbiased track fits. 
...
The root mean square of the width of the pull distributions over all planes is 1.8\,\% and 1.7\,\% for the narrow and the wide geometry, respectively, for biased track fits using beam particles with 6\,GeV. 
...
\noindent
This parameter is assumed constant as a function of sensor threshold and geometry. 
Since the intrinsic resolution is a priori independent of the plane spacing $\dz$,
 a comparison at two precisely measured spacings allows for a calibration of the amount of scattering at a given beam energy used as input to GBL. 
...
As described in section~\ref{sec:resmultiple}, the amount of multiple scattering is determined by varying $\kappa$ iteratively, until the resulting resolution for the two considered geometries agree. 
