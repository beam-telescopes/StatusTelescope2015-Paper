...
\left( \frac{\theta_{\textrm{i,j}} - \theta_{i-1,j}}{\Delta \Theta_{i,j}} \right)^2 \,.
\end{equation}
...
\\comments hj: \\
- Is this done after track fitting? so tracks are not altered during this process?\\
- Are you really using $\sigma_{i,j}$? Or just one $\sigma_{\textrm{intrinsic}}$?\\
- same for $\Delta \Theta_{i,j}$. Shouldnt it be just one $\Theta_0$ from Eq.~(4) ?\\
...
\begin{equation}
\Delta \chi^2_{i,j} = \sum_{k = 1; k \neq i}^{n}\left( \frac{y_{k,j} - p_{k,j}}{\sigma_{\textrm{intrinsic}}} \right)^2 +
\left( \frac{\theta_{k,j} - \theta_{k-1,j}}{\Theta_0} \right)^2 \, ??
\end{equation}

\noindent
- Ich verstehe den chi2-Teil leider noch nicht :/\\
...
Some considerations:\\
7 dimensions: dz, dz2, dz3, $\sigma_{reso}$, $\epsmimo = \frac{X_{\textrm{M26}}}{X_0}$, $\epsdut = \frac{X_{\textrm{DUT}}}{X_0}$,  p in GeV\\
...
\begin{figure}[tbp]
  ...
  (B) The pointing resolution for 5\,GeV (red) and 120\,GeV (black) is shown as a function of $\epsdut$ for a 6-plane set-up.}
