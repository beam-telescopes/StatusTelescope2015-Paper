...
A valid triplet is found, if a matching hit in plane\,1 is present within a matching distance of $\unit{100}{\upmu\meter}$, non-valid triplets are discarded.  
...
\begin{itemize}
 ...
 \item Corrections, i.e.\ the offsets between the initial trajectory and the fitted trajectory, are small and the kink angles are to good approximation linear functions of the these corrections,
  c.f.\ equation (3) in reference~\cite{Blobel20111760} and equation (4) in~\cite{Blobel-2006}.
...
In order to estimate the effect of the chosen fit range on the derived intrinsic resolution, the fit range is varied up and down by one standard deviation around a centre value of 2 standard deviations. 
...
The residual and pull distributions have been obtained for every sensor plane as described in sections~\ref{sec:datura-nodut}, \ref{sec:resmultiple}, and~\ref{sec:meth}
 for both methodologies, biased and unbiased track fits~\cite{jansen_2016_49065}. 
...
%The energy dependent width of the scattering angle distribution increase the uncertainty with decreasing energy, cf.\.equation~\ref{eq:multiplescattering}. 
...
As described in section~\ref{sec:meth}, the tuned amount of multiple scattering used as input to GBL
 is determined by varying $\kappa$ iteratively until the resulting intrinsic resolution for the two considered geometries agree. 
...
The systematic uncertainty on $\kappa$ includes uncertainties on the beam energy, the fit range and the rms($\pb$), c.f.\ table~\ref{tab:uncerts}. 
...
The uncertainty is purely of systematic nature and is extracted using table~\ref{tab:uncerts}. 
The result implies that the average cluster contains more than one pixel and that charge sharing is the underlying cause for an intrinsic resolution better than the binary one. 
Indeed, the average clustersize at $\noise = 6$ is about 3.3\,pixel.
...
This value of $\kappa$ has also been used for the predictions in figures~\ref{fig:smiley} (A) and (B). 
...
For thresholds $\noise = 5$ and $\noise = 6$ the efficiency is measured to $99.6\,\%$ and $99.4\,\%$, respectively, and
 the efficiency varies by less than 0.1\,\% between different geometries and energy settings. 
With increasing threshold, the efficiency decreases to about $82\,\%$ at $\noise = 12$. 
Towards lower thresholds, the efficiency also decreases, which seems counter-intuitive, but is related to an increasing number of hot pixels. 
An observed decrease in efficiency by 3.5\,\% from threshold $\noise = 5$ to $\noise = 3$ can be reasonably appointed to the about 1\,\% hot pixels in a plane at threshold $\noise = 3$,
 since all clusters containing at least one hot pixel are removed from the analysis. 
...
\begin{equation}
 \sigmat(\zdut) = \left( 1.83\,\pm\,0.03\right)\,\upmu\meter %1.81435 1.7846 1.75478. at SPS 1.32393 1.35824 1.28967
\end{equation}
...
For instance for a DUT with a material budget of $\epsdut = 0.001$ (solid lines) at 5\,GeV, a narrow configuration shows an better track resolution for $\dzdut < 74\,\milli\meter$,
 while a wide configuration is favoured  for $\dzdut > 74\,\milli\meter$. %actually 82.something
