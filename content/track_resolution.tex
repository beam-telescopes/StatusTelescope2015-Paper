The results presented in this section have been obtained using the EUTelescope software (cf.\ Section~\ref{sec:offline}).
The setup does not include any additional DUT, only six $\Mimosa$ telescope planes are included and unbiased track residuals for each telescope plane are calculated.
...
In a separate iteration for every telescope plane a track fit is performed excluding the plane under investigation and thus only using data from the five remaining planes for tracking.
The tracks are extrapolated to the plane under investigation, and the residual distance to its hits as well as its tracking efficiency are calculated.
