...
For a composition of scatterers $\varepsilon_i$, cf.\ section~\ref{sec:sensors}, the variance after the last scatterer involves the sum over the material budgets and hence $\varepsilon = \sum \varepsilon_i$. 
For track fitting (with a Kalman filter or general broken lines (GBL)) the variance from all scatterers on the track has to be distributed over the individual scatterers. 
Using the material fraction as relative weight results in a formula similar to equation~(\ref{eq:multiplescattering}) except for the logarithmic correction
...
\noindent The amount of multiple scattering is calculated per sensor considering the measured sensor thickness and a $25\,\micro\meter$ thick Kapton foil on either side of each sensor.
