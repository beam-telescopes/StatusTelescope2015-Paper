...
Track fits are formed using the GBL formalism and biased track residuals, i.e.\ the distance between the track fit and the measured hit,
...
These tracks can be formed in a \textrm{biased} or in an \textrm{unbiased} form, where biased tracks include the measured hit information of every plane in contrast to the unbiased tracks,
...
\subsection{Residuals and multiple scattering}
...
The figure of merit for a beam telescope is its track resolution\footnote{also called pointing resolution} $\sigmatb$ ($\sigmatu$),
 as this defines the precision with which a particle trajectory can be determined for an biased (unbiased) track. 
...
With a known or estimated biased track resolution, i.e.\ all $\sigmai(z_i)$ are known or estimates exist,
 the width of the distribution of the residuals for a set of tracks can be expressed for all positions $z = z_i$ as
...
\noindent
If the intrinsic resolution of a DUT at $\zdut$ or a telescope plane at a position $z_i$ is not known or not included in the fit,
 the width of the distribution of the unbiased residuals for a set of tracks at these positions reads~\cite{ref:eudetreport200902}
...
\noindent
These distributions are a function of the track resolution and therefore have the same dependencies.
...
The pull of a track is defined as the ratio of the biased residual over the predicted residual uncertainty

\begin{equation}
 \textrm{pull}_{\textrm{b}} \equiv p_{\textrm{b}} = \frac{\rbiased}{\sqrt{\sigmai^2 - \sigmatb^2}}.
 \label{eq:pull}
\end{equation}
...
The latter is caused by either overestimated material budget or overestimated deflection angles from multiple scattering. 
 
Various methods exist to perform the calculation of the track resolution. 
In the formalism of GBL,
 the track resolution results from a single global fit to all measured hits including their measurement uncertainties (intrinsic resolution) and scattering angle uncertainties along the trajectory. 
% 
...
As the beam particles interact with the sensors and the air, contributions to the amount of multiple scattering from both have to be considered,
 with a dependence of the air contribution on the distance $\dz$. 
...
In reality, the material thicknesses and the amount of multiple scattering therein are only known with a certain accuracy affecting the predicted residual uncertainty. 
%Especially the Highland formula is only accurate to about 11\,\%. 

- Calculate sqrt(...) for biased and unbiased for the following sys uncerts separately
...
In case the Highland formula underestimates the actual straggling of beam particles in the given material, the predicted tracking resolution is systematically affected, and therefore the pull distribution. 
...
\noindent
The value $\kappa$ is found when the set of intrinsic resolutions for the two plane spacing match within the uncertainties from the material budget and the beam energy. 
Practically, an agreement of better than 2\,\% is accepted herein. 
...
This procedure converges towards the true intrinsic resolution and yields compatible results for the two different geometries for an appropriate choice of $\kappa$. 
...
\begin{figure}[tbp]
  ...
  Biased residual distributions measured with the $\Datura$ telescope at 6\,GeV. 
...
(2) The biased tracking resolution tends towards the intrinsic resolution for increased scattering. 
