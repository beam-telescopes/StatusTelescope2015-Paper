...
A busy signal is accepted by the TLU from each integrated DAQ individually vetoing the issue of the next trigger if the busy signal is high. 
More details are available in Ref.~\cite{TLU,EUDET-2007-20}.
...
The handling of the trigger/busy signals can be done in three handshake modes, one being the \textit{no-handshake mode},
...
The TLU then de-asserts the trigger, and waits for busy going low on the busy line.
