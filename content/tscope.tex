...
In the Cartesian, right-handed coordinate system chosen, the $x$-direction points horizontally and the $z$-direction along the beam.
...
The $\Mimosa$ sensors used for precise spacial measurements of particle trajectories are manufactured with the 350\,nm CMOS technology~\cite{HuGuo2010480}. 
...
The $\Mimosa$ sensors have been thinned down to $\unit{50}{\upmu\meter}$ and are protected on each side by a $\unit{25}{\upmu\meter}$ lightproof Kapton foil. 
...
At this threshold, the average noise occupancy is measured to be smaller than $2\cdot10^{-5}$ pixel per read-out frame at room temperature.
...
Furthermore, the jigs are cooled keeping the $\Mimosa$ sensors at a constant temperature for stable operation.
